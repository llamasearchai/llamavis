...
class DataValidator:
    ...
    @staticmethod
    def validate_for_chart(data: Any, chart_type: str) -> Tuple[bool, Optional[str]]:
        """
        ...
        if chart_type in ["line", "bar", "radar"]:
            ...
            for dataset in data["datasets"]:
                ...
                if len(dataset["data"]) != len(data["labels"]):
                    return (
                        False,
                        f"Dataset has {len(dataset['data'])} values but there are {len(data['labels'])} labels",
                    )
            ...
            if len(data["values"]) != len(data["labels"]):
                return (
                    False,
                    f"There are {len(data['values'])} values but {len(data['labels'])} labels",
                )
    ...
    @staticmethod
    def validate_for_network(data: Any) -> Tuple[bool, Optional[str]]:
        """
    ...
    @staticmethod
    def validate_for_3d(data: Any, vis_type: str) -> Tuple[bool, Optional[str]]:
        """
        ...
        if vis_type == "scatter3d":
            ...
            return (
                False,
                "Surface3D data must be a function or a dictionary with 'values' key",
            )
...
class DataTransformer:
    ...
    def from_dataframe(
        ...
        if chart_type in ["line", "bar", "radar"]:
            ...
            for col in y_columns:
                datasets.append({"label": col, "data": df[col].tolist()})
            ...
            return {"labels": df[x_column].tolist(), "values": df[y_col].tolist()}
            ...
            for col in y_columns:
                ...
                for _, row in df.iterrows():
                    data.append({"x": float(row[x_column]), "y": float(row[col])})
            ...
            if len(y_columns) < 2:
                raise ValueError(
                    "Heatmap requires at least two y columns (one for categories, one for values)"
                )
            ...
            pivot_data = df.pivot(
                index=y_column, columns=x_column, values=value_column
            ).fillna(0)
            ...
            raise ValueError(
                f"DataFrame to {chart_type} conversion requires additional parameters. Use from_dataframe_to_network() instead."
            )
    ...
    @staticmethod
    def from_json(
        json_data: Union[str, Dict[str, Any]], chart_type: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def from_csv(
        ...
        **pandas_kwargs,
        ...
        return DataTransformer.from_dataframe(df, x_column, y_columns, chart_type)
