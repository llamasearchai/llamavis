...
def generate_contrasting_colors(count: int, base_hue: Optional[float] = None) -> List[str]:
    """
...
def format_number(value: Union[int, float], precision: int = 2) -> str:
    ...
    """
    if abs(value) < 1000:
        return f"{value:.{precision}f}".rstrip('0').rstrip('.') if '.' in f"{value:.{precision}f}" else f"{value}"
...
def generate_color_scale(
    ...
    for i in range(segments):
        ...
        if segment_colors == 1:
            ...
            result.extend(interpolate_colors(
                base_colors[i],
                base_colors[i+1],
                segment_colors
            )[:-1])  # Exclude the last color to avoid duplication
    ...
    for _ in range(n_colors):
        ...
        while attempts < max_attempts:
            ...
            for existing_color in result:
                ...
                if (h_dist < min_distance and 
                    s_dist < min_distance and 
                    v_dist < min_distance):
                    too_close = True
                    break
