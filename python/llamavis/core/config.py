...
class VisualizationConfig:
    ...
    def __init__(
        ...
        **additional_options
        ...
        self.chart_type = ChartType(chart_type) if isinstance(chart_type, str) else chart_type
        
        # Handle color palette
        if isinstance(color_palette, str):
            self.color_palette = COLOR_PALETTES.get(color_palette, COLOR_PALETTES["llamasearch"])
        else:
        ...
        self.interactions = interactions or [Interaction.HOVER, Interaction.CLICK, Interaction.ZOOM]
        ...
        self.background_color = background_color or (
            "#ffffff" if self.theme == ThemeType.LIGHT else 
            "#1a1a1a" if self.theme == ThemeType.DARK else 
            "#f6f6f9"  # LlamaSearch light background
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "chart_type": self.chart_type.value if isinstance(self.chart_type, Enum) else self.chart_type,
            "color_palette": self.color_palette,
            "show_legend": self.show_legend,
            "show_axes": self.show_axes,
            "show_grid": self.show_grid,
            "interactive": self.interactive,
            "interactions": [i.value if isinstance(i, Enum) else i for i in self.interactions],
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'VisualizationConfig':
        ...
        standard_params = {
            "theme", "chart_type", "color_palette", "show_legend", "show_axes", 
            "show_grid", "interactive", "interactions", "animation", 
            "animation_duration", "background_color", "axis_labels", "font_size", 
            "font_family", "title_font_size", "responsive", "smooth_curves", 
            "tooltip_format", "margin"
        }
        
        additional_options = {k: v for k, v in config_dict.items() if k not in standard_params}
